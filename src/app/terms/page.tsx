...
export default function TermsPage() {
  const [policy, setPolicy] = useState<LegalDocument>(() => legalService.getDefaults('terms'));
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    legalService.getLegalContent('terms').then((data) => {
      setPolicy(data);
      setLoading(false);
    });
  }, []);
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-8 sm:p-12 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
            <span className="text-[10px] font-bold uppercase tracking-widest text-[#1540a8] bg-blue-50 px-2.5 py-1 rounded-md">
              {policy.categoryBadge || 'Legal Governance & Bylaws'}
            </span>
            {policy.lastUpdated && (
